...
public class CLI {
    ...
    @Option(name = "-help", usage = "Print usages.") private boolean HELP = false;

    @Option(name = "-rounds", usage = "Number of rounds.") private int ROUNDS = 1000;

    @Option(name = "-numPartitions", usage = "Number of partitions.") private int NUM_PARTITIONS = 4;

    @Option(name = "-uniformRandSampleSize", usage = "Uniform random sample size.") private int UNIFORM_RAND_SAMPLE_SIZE = 6;

    @Option(name = "-temp", usage = "Simulated annealing temperature.") private float TEMPERATURE = 2;

    @Option(name = "-delta", usage = "Simulated annealing delta.") private float DELTA = (float) 0.003;
    ...
    @Option(name = "-graphInitColorSelectionPolicy", usage = "Initial color celection policy. Supported, RANDOM, ROUND_ROBIN, BATCH") private String GRAPH_INIT_COLOR_SELECTION_POLICY = "ROUND_ROBIN";
    private GraphInitColorPolicy graphInitColorSelectionPolicy = GraphInitColorPolicy.ROUND_ROBIN;

    @Option(name = "-nodeSelectionPolicy", usage = "Node selection plicy. Supported, RANDOM, LOCAL, HYBRID") private String NODE_SELECTION_POLICY = "HYBRID";
